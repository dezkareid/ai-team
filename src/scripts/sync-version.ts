...
export type SyncStrategy = 'gemini' | 'claude';
...
function getVersion(json: any, strategy: SyncStrategy): string {
  if (strategy === 'claude') {
...
function setVersion(json: any, version: string, strategy: SyncStrategy): void {
  if (strategy === 'claude') {
...
export function syncVersion(
  ...
  try {
    ...
    const currentVersion = getVersion(targetJson, strategy);
    if (currentVersion !== packageJson.version) {
      const oldVersion = currentVersion;
      setVersion(targetJson, packageJson.version, strategy);
...
async function run() {
  const isDist = __dirname.includes(path.join('dist', 'scripts'));
  ...
  const targets: Array<{ path: string; strategy: SyncStrategy; silent?: boolean }> = [
    ...
    { path: path.join(rootDir, '.claude-plugin/marketplace.json'), strategy: 'claude', silent: true },
  ];
